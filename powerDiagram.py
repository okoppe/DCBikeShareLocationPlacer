#Code adapted from https://gist.github.com/marmakoide/45d5389252683ae09c2df49d0548a627#file-laguerre-voronoi-2d-py
...
def displayV(S, R, tri_list, voronoi_cell_map):
	# Setup
...
def createLVDiagram(points=None, weights=None):
    
    sample_count = 32
    
    weights = .8 * numpy.random.random(len(points)) + .2 
    
    # Compute the power triangulation of the circles
    tri_list, V = get_power_triangulation(points, weights)
    
    # Compute the Voronoi cells
    voronoi_cell_map = get_voronoi_cells(points, V, tri_list)
    
    # Display the result
    return displayV(points, weights, tri_list, voronoi_cell_map)

def transform(array):
    # Find the minimum and maximum values along each axis
    min_x, min_y = 38.766844, -77.368416

    # Shift the coordinates to make bottom-left (0, 0)
...
def undoTransformC(line_collection):
    # Create an empty list to store the transformed line segments
    original_segments = []
    ...
    for line in line_collection.get_segments():
        ...
        # Append the original segment to the list
        original_segments.append(original_segment)

    # Reconstruct the LineCollection with the original line segments
    original_line_collection = LineCollection(original_segments)

    return original_line_collection

def add_line_collection_to_map(m, line_collection, color='blue', weight=2):
    for line in line_collection.get_segments():
        coordinates = [(point[0], point[1]) for point in line] 
        folium.PolyLine(
